...
class Control:
    ...
    def __parking__(self):
        ...
        if self.psit == 1:
            ...
            elif 100 <= (self.pt2 - self.pt1) < 249:  # 변경할 때 걸리는 엔코더 초과값 계산 및 보정 필요(593)
                self.steer = 1970

            if (self.pt2 - self.pt1) >= 249:
                self.steer = 0
                self.speed = 0
                self.brake = 60
    ...
    def __uturn__(self):
        ...
        if self.usit == 1:
            ...
            elif 100 <= (self.ct2 - self.ct1) < 603:
                self.steer = -1970

            if (self.ct2 - self.ct1) >= 603:
                self.steer = 0
                self.speed = 0
                self.brake = 60
